...
implements Runnable, SiteTransactionConnection, SiteProcedureConnection
{
	...
	public void run() {
		...
		try {
			...
			while (m_shouldContinue) {
				...
				//hostLog.l7dlog(Level.INFO, "before poll!!!!:count" + m_transactionQueue.size(), null);
				TransactionState currentTxnState = (TransactionState)m_transactionQueue.poll();
				//hostLog.l7dlog(Level.INFO, "after poll!!!!:count" + m_transactionQueue.size(), null);
	...
	private void completeTransaction(TransactionState txnState) {
		...
		if (txnState instanceof PhysicalLogUpdateTxnState ) {
			if (txnState.coordinatorSiteId == m_siteId) {
				...
				hostLog.l7dlog(Level.INFO, "before recvBlocking", null);
				while(replies < ptask.getOtherSiteCount()+1) {
					VoltMessage m = m_mailbox.recvBlocking(5);
					if (m == null) {
						tick();
						continue;
					} else {
						//hostLog.l7dlog(Level.INFO, "Got one response", null);
						if (m instanceof PhysicalLogResponseMessage) {
							hostLog.l7dlog(Level.INFO, "Got one response: PhysicalLogRwsponse", null);
							if(((PhysicalLogResponseMessage) m).getTxnId() == txnState.txnId) {
								replies ++;
								continue;
							} else {
								//TODO: not matched txnId reply, ignore it?
							}
						} else {
							handleMailboxMessage(m);
						}
					}
				}

				((PhysicalLogUpdateTxnState)txnState).finishTransaction();
	...
	private void handleMailboxMessage(VoltMessage message) {
		if (m_recovering == true && m_recoveryProcessor == null && m_currentTransactionState != null) {
			...
			//hostLog.l7dlog( Level.INFO, "entering wrong handleMailboxMessage", null);
			m_recoveryMessageHandler.handleMessage(message, m_currentTransactionState.txnId);
		} else {
			//hostLog.l7dlog( Level.INFO, "entering handleMailboxMessage", null);
			handleMailboxMessageNonRecursable(message);
		}
	}
	...
	recursableRun(TransactionState currentTxnState)
	{
		do
		{
			...
			if (currentTxnState.doWork(m_recovering)) {
				...
				//hostLog.l7dlog(Level.INFO, "1", null);
